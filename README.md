...
![GitHubActionsWatch Demo](demo_githubactionswatch.png)
...
GitHubActionsWatch is fully local. It does not collect, store, or transmit any data to external servers. Your GitHub Personal Access Token is stored exclusively in the macOS Keychain. Preferences are saved locally via UserDefaults. There is no analytics, no telemetry, no phone-home behavior of any kind.

## Requirements

- macOS 14.0 or later
- Swift 6.0+ (to build from source)
- A [GitHub Personal Access Token (fine-grained)](https://github.com/settings/tokens?type=beta) with **read-only** access to **Actions**. You can scope it to all repositories or only the ones you want to monitor — GitHubActionsWatch lets you pick specific repos from within the app
...
1. Launch GitHubActionsWatch — it appears in your menu bar
2. Click the icon and sign in with your GitHub Personal Access Token
3. Select the repositories you want to monitor
4. GitHubActionsWatch will poll GitHub Actions and update the menu bar indicator in real time
...
Because GitHubActionsWatch is currently a free, open-source project, the downloaded app is not signed with a paid Apple Developer Developer ID.

Because of this built-in macOS security feature, the system will ask for your permission the first time GitHubActionsWatch tries to retrieve your GitHub token from the Keychain.
...
This project is a fork of [Octowatch](https://github.com/hbourget/octowatch) by [@hbourget](https://github.com/hbourget). Special thanks for the original idea and implementation that made this project possible.
